...
#include "binarySearchTreeTest.c"
...
struct hash hashes[MAXHASH + 1] = { { "CRC", 0, GCRY_MD_CRC32, 4 }, { "MD5", 1,
		GCRY_MD_MD5, 16 }, { "RIPEMD", 2, GCRY_MD_RMD160, 20 }, { "SHA256", 3,
		GCRY_MD_SHA256, 32 }, { "SHA384", 4, GCRY_MD_SHA384, 48 }, { "SHA512",
		5, GCRY_MD_SHA512, 64 }, { "SHA3_256", 6, GCRY_MD_SHA3_256, 32 }, {
		"SHA3_512", 7, GCRY_MD_SHA3_512, 64 }, { "SHAKE128_16", 8,
		GCRY_MD_SHAKE128, 16 }, /* XOF function */
...
struct encryption encryptions[MAXENC + 1] = { { "NONE", 0, 0, 0, 0 }, /* encryption not used */
{ "AES128", 1, GCRY_CIPHER_AES128, 16, 16 }, { "AES256", 2, GCRY_CIPHER_AES256,
		32, 16 }, };
	...
	if (!INFO) {
		...
		if (!DECOMPRESS && !TEST_ONLY) {
			print_verbose("Compression mode is: %s",
					(LZMA_COMPRESS ? "LZMA" : (LZO_COMPRESS ? "LZO\n" :	// No Threshold testing
					(BZIP2_COMPRESS ? "BZIP2" : (ZLIB_COMPRESS ? "GZIP\n" :// No Threshold testing
					(ZPAQ_COMPRESS ? "ZPAQ" : (NO_COMPRESS ? "RZIP pre-processing only" : "wtf")))))));
			...
			if (ZPAQ_COMPRESS)
				print_verbose(
						"ZPAQ Compression Level: %'d, ZPAQ initial Block Size: %'d\n",
						control->zpaq_level, control->zpaq_bs);
			if (FILTER_USED) {
				print_output("Filter Used: %s",
						((control->filter_flag == FILTER_FLAG_X86) ? "x86" : ((control->filter_flag == FILTER_FLAG_ARM) ? "ARM" : ((control->filter_flag == FILTER_FLAG_ARMT) ? "ARMT" : ((control->filter_flag == FILTER_FLAG_PPC) ? "PPC" : ((control->filter_flag == FILTER_FLAG_SPARC) ? "SPARC" : ((control->filter_flag == FILTER_FLAG_IA64) ? "IA64" : ((control->filter_flag == FILTER_FLAG_DELTA) ? "Delta" : "wtf?"))))))));
			...
			if (!control->window && !UNLIMITED) {
				...
				print_verbose(
						"Heuristically Computed Compression Window: %'"PRId64" = %'"PRId64"MB\n",
...
static struct option long_options[] = { { "bzip2", no_argument, 0, 'b' }, /* 0 */
{ "check", no_argument, 0, 'c' }, { "comment", required_argument, 0, 'C' }, {
		"decompress", no_argument, 0, 'd' }, { "delete", no_argument, 0, 'D' },
		{ "encrypt", optional_argument, 0, 'e' }, /* 5 */
		{ "emethod", required_argument, 0, 'E' },
		{ "force", no_argument, 0, 'f' }, { "gzip", no_argument, 0, 'g' }, {
				"help", no_argument, 0, 'h' }, { "hash", optional_argument, 0,
				'H' }, /* 10 */
		{ "info", no_argument, 0, 'i' }, { "keep-broken", no_argument, 0, 'K' },
		{ "lzo", no_argument, 0, 'l' }, { "level", optional_argument, 0, 'L' },
		{ "maxram", required_argument, 0, 'm' }, /* 15 */
		{ "no-compress", no_argument, 0, 'n' }, { "nice-level",
		required_argument, 0, 'N' }, { "outfile", required_argument, 0, 'o' }, {
				"outdir", required_argument, 0, 'O' }, { "threads",
		required_argument, 0, 'p' }, /* 20 */
		{ "progress", no_argument, 0, 'P' }, { "quiet", no_argument, 0, 'q' }, {
				"rzip-level", required_argument, 0, 'R' }, { "suffix",
		required_argument, 0, 'S' }, { "test", no_argument, 0, 't' }, /* 25 */
		{ "threshold", optional_argument, 0, 'T' }, { "unlimited", no_argument,
				0, 'U' }, { "verbose", no_argument, 0, 'v' }, { "version",
		no_argument, 0, 'V' }, { "window", required_argument, 0, 'w' }, /* 30 */
		{ "zpaq", no_argument, 0, 'z' }, { "fast", no_argument, 0, '1' }, {
				"best", no_argument, 0, '9' }, { "lzma", no_argument, 0, 0 }, /* 34 - begin long opt index */
		{ "dictsize", required_argument, 0, 0 }, /* 35 */
		{ "zpaqbs", required_argument, 0, 0 }, { "x86", no_argument, 0, 0 }, /* 37 */
		{ "arm", no_argument, 0, 0 }, { "armt", no_argument, 0, 0 }, { "ppc",
		no_argument, 0, 0 }, /* 40 */
		{ "sparc", no_argument, 0, 0 }, { "ia64", no_argument, 0, 0 }, {
...
static const char *loptions =
		"bcC:dDe::E:fghH::iKlL:nN:o:O:p:PqR:S:tT::Um:vVw:z?";
...
int main(int argc, char *argv[]) {
	...
	while ((c = getopt_long(argc, argv, loptions, long_options, &long_opt_index))
			!= -1) {
		switch (c) {
		case 'b':
		case 'g':
		...
		case 'H':
			...
			if (optarg) {
				...
				if (i < 1 || i > MAXHASH)
					fatal(
							"Hash codes out of bounds. Must be between 1 and %d.\n",
							MAXHASH);
				control->hash_code = i;
			}
			break;
		...
		case 'R':
			...
			if (*endptr)
				fatal("Extra characters after rzip compression level: \'%s\'\n",
						endptr);
			if (control->rzip_compression_level < 1
					|| control->rzip_compression_level > 9)
		...
		case 'o':
			...
			if (unlikely(STDOUT))
				fatal(
						"Cannot specify an output filename when outputting to stdout\n");
		...
		case 'O':
			...
			if (unlikely(STDOUT))
				fatal(
						"Cannot specify an output directory when outputting to stdout\n");
		...
		case 'S':
			if (control->outname)
				fatal(
						"Specified output filename already, can't specify an extension.\n");
			if (unlikely(STDOUT))
				fatal(
						"Cannot specify a filename suffix when outputting to stdout\n");
			control->suffix = strdup(optarg);
			break;
		case 't':
			if (control->outname)
				fatal(
						"Cannot specify an output file name when just testing.\n");
			if (!KEEP_FILES)
				fatal(
						"Doubt that you want to delete a file when just testing.\n");
		...
		case 0: /* these are long options without a short code */
			if (FILTER_USED && long_opt_index >= FILTERSTART)
				...
				case LONGSTART + 1:
					...
					if (!LZMA_COMPRESS)
						print_err(
								"--dictsize option only valid for LZMA compression. Ignorred.\n");
					else {
						ds = strtol(optarg, &endptr, 10);
						if (*endptr)
							fatal(
									"Extra characters after dictionary size: \'%s\'\n",
									endptr);
						if (ds < 0 || ds > 40)
							fatal(
									"Dictionary Size must be between 0 and 40 for 2^12 (4KB) to 2^31 (4GB)\n");
				...
				case LONGSTART + 2:
					if (!ZPAQ_COMPRESS)
						print_err(
								"--zpaqbs option only valid for ZPAQ compression. Ignored.\n");
					else {
						ds = strtol(optarg, &endptr, 10);
						if (*endptr)
				...
				case FILTERSTART + 6:
					...
					if (optarg) {
						...
						if (*endptr)
							fatal(
									"Extra characters after delta offset: \'%s\'\n",
									endptr);
						if (i < 1 || i > 32)
							fatal(
									"Delta offset value must be between 1 and 32\n");
...
//	}
	...
	if ((control->flags & FLAG_THRESHOLD)
			&& (FILTER_USED || ZLIB_COMPRESS || LZO_COMPRESS || NO_COMPRESS)) {
		print_output(
	...
	if (nice_set) {
		if (!NO_COMPRESS) {
			...
			if (unlikely(
					setpriority(PRIO_PROCESS, 0, control->nice_val/2) == -1)) {
				print_err(
						"Warning, unable to set nice value %'d...Resetting to %'d\n",
			...
			if (unlikely(
					setpriority(PRIO_PROCESS, 0, control->nice_val) == -1)) {
				print_err(
						"Warning, unable to set nice value %'d...Resetting to %'d\n",
						control->nice_val, control->current_priority);
				setpriority(PRIO_PROCESS, 0,
...
//	folder_test1();
	...
	fileCache_init(rzip_files);

	control->rzip_files = rzip_files;
...
//		char *infile = NULL;
		...
		if (i < argc) {
			...
			if (DECOMPRESS || TEST_ONLY) {
				...
				if(i>0) {
					// decompress
					decompress_addFilter(rzip_files, argv[i]);
				} else {
				...
				// compress
				files_addPath(rzip_files, argv[i]);
			}
		}
...
//		control->infile = infile;
	...
	if (!FORCE_REPLACE) {
		...
		if (!TEST_ONLY && STDOUT && isatty(fileno((FILE*) stdout))) {
			print_err(
					"Will not write stdout to a terminal. Use -f to override.\n");
			usage();
			exit(1);
		}
	}
	...
	if (CHECK_FILE) {
		if (!DECOMPRESS) {
			...
			print_err(
					"Can't check file written when writing to stdout. Checking disabled.\n");
			control->flags &= ~FLAG_CHECK;
		}
	}
	...
	if (unlikely(STDIN && ENCRYPT && control->passphrase == NULL))
		fatal(
				"Unable to work from STDIN while reading password. Use -e passphrase.\n");
