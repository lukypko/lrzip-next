...
static void sliding_next_tag2(rzip_control *control, struct rzip_state *st, i64 p, tag *t)
{
	uchar u;

	sliding_next_tag2_count++;

	u=files_getManagedBufferByte(control->rzip_files->bufferNextTag1, p-1);
//	u = fileCache_getByte(control->rzip_files, p - 1);
	*t ^= st->hash_index[u];

	//u = fileCache_getByte(control->rzip_files, p + MINIMUM_MATCH - 1);
	u=files_getManagedBufferByte(control->rzip_files->bufferNextTag2, p + MINIMUM_MATCH - 1);
	*t ^= st->hash_index[u];
}

static i64 sliding_full_tag2_count = 0;
static tag sliding_full_tag3(rzip_control *control, struct rzip_state *st, i64 p)
{
	tag ret = 0;
	int i;
	uchar u;

	//printf("sliding_full_tag3");

	sliding_full_tag2_count++;

	for(i=0;i < MINIMUM_MATCH;) {

		files_mmapOffset(control->rzip_files->bufferFullTag, p+i);
		//struct rzip_files_buffer *buffer = fileCache_remapBuffer(control->rzip_files, p + i);
		int64_t availableBuffer = files_min(control->rzip_files->bufferFullTag->end - (p+i), MINIMUM_MATCH - i);
		if(availableBuffer==0) {
			printf("sliding_full_tag3: availableBuffer==0\n");
		}
		for (; availableBuffer > 0; availableBuffer--) {

			u = files_getDirectBufferByte(control->rzip_files->bufferFullTag, p + i);
			ret ^= st->hash_index[u];
			i++;
		}
	}
 ...
 */
static i64 sliding_match_len2(rzip_control *control, struct rzip_state *st, i64 p0, i64 op, i64 end, i64 *rev)
{
	i64 p, len;

	//printf("sliding_match_len2");
	...
	p = p0;
	while (p < end && fileCache_getByte(control->rzip_files, p) == fileCache_getByte(control->rzip_files, op)) {
		p++;
		op++;
	}
	len = p - p0;
	...
	while (p > end && op > 0 && fileCache_getByte(control->rzip_files, op - 1) == fileCache_getByte(control->rzip_files, p - 1)) {
		op--;
		p--;
	}
...
static i64 sliding_match_len3(rzip_control *control, struct rzip_state *st, i64 p0, i64 op, i64 end, i64 *rev)
{
	i64 p, len;
	...
	p = p0;
	while (p < end && files_getManagedBufferByte(control->rzip_files->bufferMatchLen1, p) == files_getManagedBufferByte(control->rzip_files->bufferMatchLen2, op)) {
		p++;
		op++;
	}
	len = p - p0;
	...
	while (p > end && op > 0 && files_getManagedBufferByte(control->rzip_files->bufferMatchLen3, op - 1) == files_getManagedBufferByte(control->rzip_files->bufferMatchLen4, p - 1)) {
		op--;
		p--;
	}
...
static i64 sliding_match_len_mmap1_count = 0;
static i64 sliding_match_len_mmap2_count = 0;

static i64 sliding_match_len4(rzip_control *control, struct rzip_state *st, i64 p0, i64 op, i64 end, i64 *rev)
{
	i64 p, len;
	...
	struct rzip_files_buffer *buffer1=control->rzip_files->bufferMatchLen1;
	struct rzip_files_buffer *buffer2=control->rzip_files->bufferMatchLen2;
	int64_t counter;

	while (p < end) {

		files_mmapOffset(buffer1, p);
		counter = files_min(buffer1->end - p, end-p);

		files_mmapOffset(buffer2, op);
		counter = files_min(buffer2->end - op, counter);

//		if(counter==0) {
//			printf("sliding_match_len4: counter1==0\n");
//		}

		sliding_match_len_mmap1_count++;

		// while (p < end && fileCache_getByte(control->rzip_files, p) == fileCache_getByte(control->rzip_files, op))
		while(counter-->0) {
			if(files_getDirectBufferByte(buffer1, p)!=files_getDirectBufferByte(buffer2, op)) {
				goto endForward;
			}
			p++;
			op++;
	...
	buffer1=control->rzip_files->bufferMatchLen3;
	buffer2=control->rzip_files->bufferMatchLen4;
...
//	}
	...
	while (p > end && op>0) {
		...
		files_mmapOffset(buffer1, op-1);
		// there is `op` and not `op-1` because byte on index "zero" is allowed too
		counter = files_min(op-buffer1->offset, op);

		files_mmapOffset(buffer2, p-1);
		// there is `p` and not `p-1` because byte on index "zero" is allowed too
		counter = files_min(files_min(p-buffer2->offset, p-end), counter);

		sliding_match_len_mmap2_count++;

			// while (p > end && op > 0 && fileCache_getByte(control->rzip_files, op - 1) == fileCache_getByte(control->rzip_files, p - 1))
		while(counter-->0) {
			if(files_getDirectBufferByte(buffer1, op-1)!=files_getDirectBufferByte(buffer2, p-1)) {
				goto endBackward;
			}
			op--;
			p--;
...
static i64 find_best_match_counter = 0;
static inline i64
...
static inline void
rzip_chunk(rzip_control *control, struct rzip_state *st, int fd_out,
...
//	}
	...
	control->match_len = &sliding_match_len4;
...
//		}
		...
		if (st->chunk_size == len)
			control->eof = 1;
		rzip_chunk(control, st, fd_out, offset, pct_base, pct_multiple);
	...
	printf("find_best_match_counter=%'lu\n", find_best_match_counter);
	printf("sliding_match_len_mmap1_count=%'lu\n", sliding_match_len_mmap1_count);
	printf("sliding_match_len_mmap2_count=%'lu\n", sliding_match_len_mmap2_count);
