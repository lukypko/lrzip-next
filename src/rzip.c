...
static tag sliding_full_tag2(rzip_control *control, struct rzip_state *st,
	...
	for (i = 0; i < MINIMUM_MATCH; i++) {
		u = fileCache_getByte(control, p + i);
//		u = sliding_get_sb(control, p + i);
		ret ^= st->hash_index[u];
	}
	return ret;
...
//			&& *sliding_get_sb(control, p) == *sliding_get_sb(control, op)) {
	while (p < end
			&& fileCache_getByte(control, p) == fileCache_getByte(control, op)) {
		p++;
		op++;
	}
	len = p - p0;
	...
	while (p > end && op > 0
			&& fileCache_getByte(control, op - 1)
					== fileCache_getByte(control, p - 1)) {
		op--;
		p--;
	}
...
//		files_mmapOffset(control, lowBuffer, ++p);
		...
		if (p > cksum_limit) {
			...
			control->do_mcpy(control, control->checksum.buf, cksum_limit,
					control->checksum.len);
...
//	control->do_mcpy = single_mcpy;
	control->do_mcpy = &fileCache_copyRange;
	control->next_tag = &sliding_next_tag2;
	control->full_tag = &sliding_full_tag2;
	control->match_len = &sliding_match_len2;
...
//		sb->orig_offset = offset;
	...
	printf("fileCache_copyRange_count=%'lu\n", fileCache_copyRange_count);
